...
반드시 1분 이상 읽을 수 있는 길이로 작성하되(최소 200-250단어), 인사말은 제외하고 바로 본론으로 시작하세요.
...
다음 순서로 각 항목을 상세하게 설명하여 1분 이상의 길이로 작성하세요:
...
3. 안전장비 착용 및 점검 (15초 분량)
   - 필수 안전장비 목록과 착용 방법
   - 장비 점검 포인트
   - 보호구 사용시 주의사항

4. 작업 전후 안전 절차 (15초 분량)
   - 작업 시작 전 필수 점검사항
   - 작업 중 안전 수칙
   - 작업 완료 후 정리사항

5. 비상상황 대응 및 연락처 (10초 분량)
   - 응급상황 발생시 즉시 행동요령
   - 비상연락처 정보
...
- "몇 초" 같은 시간 관련 표현 금지
- 각 항목을 충분히 상세하게 설명
- 실제 1분 이상 읽을 수 있도록 충분한 내용량 확보
- 구체적이고 실용적인 안전 지침만 포함`;

      const response = await genai.models.generateContent({
        model: "gemini-2.5-flash",
        config: {
          systemInstruction: "당신은 산업 안전 음성 안내 전문가입니다. 반드시 1분 이상 읽을 수 있는 길이(최소 200-250단어)의 상세하고 실용적인 안전 안내를 제공해야 합니다. 인사말과 마무리 인사는 완전히 제외하고 바로 본론으로 시작하세요. 시간 관련 멘트는 절대 포함하지 마세요. 각 안전 항목을 충분히 상세하게 설명하여 실제 1분 이상의 내용량을 확보하세요. 간단한 설명이 아닌 구체적이고 상세한 안전 지침을 제공하세요."
        },
        contents: prompt
      });

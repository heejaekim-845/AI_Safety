...
const SEARCH_PROFILES: Profile[] = [
  {
    ...
    include_if_any_keywords: ["전기", "고압", "절연", "변전", "GIS", "170kV", "SF6", "가스절연", "충전부", "개폐기", "감전", "전력", "전압", "전기기계기구", "특별고압", "활선", "정전", "접지", "안전거리"],
    ...
    queries: {
      ...
      regulation: [
        ...
        "특별고압 작업기준", "고압 전기안전", "충전부 접근금지", "절연장갑 착용", "전기작업 안전수칙"
      ],
      education: [
...
export function buildTargetedSearchQuery(
  ...
  const regulation = uniq([
    ...
    ...(dynamicHead.length ? [
      ...
      // 전기 설비의 경우 더 구체적이고 전문적인 규정 검색 (사고사례 수준으로 상세화)
      ...(isElectricalEquipment ? [
        `${nameTokens.join(" ")} 전기작업 안전기준`,
        `${nameTokens.join(" ")} 감전방지 규정`,
        `${nameTokens.join(" ")} 정전작업 규정`,
        `고압 전기설비 안전거리 규정`,
        `전기설비 접지 절차`,
        `절연용 보호구 착용 규정`,
        `전기기계기구 작업기준`,

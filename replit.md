...
- **Code Cleanup Phase 2 Complete (August 20, 2025)**: Major cleanup of ai-service.ts lines 32-328, removing 242 lines (11.3% reduction). Eliminated duplicate threshold functions, precision control helpers, redundant search functions, and hardcoded accident data. Simplified voice guide fallback from 25 lines to 1 line error message. Removed electrical equipment specialized code (82 lines) including hardcoded keywords, safety_rules.json file reading, and industry-specific logic. Total reduction: 867 lines (32.6% from original 2,657 lines). Unified search architecture now uses single runSearchQueries() function instead of separate vector/keyword queries.
- **Profile System Integration (August 20, 2025)**: Successfully consolidated search profile system by removing config/search-profiles.json dependency and moving all profile data into server/profiles.ts with full TypeScript type safety. Eliminated fs/path imports and JSON file reading, simplifying the architecture while maintaining all functionality. Removed 130+ lines of hardcoded regulation data from chroma-rag-service.ts, as regulation documents should be managed through vector database search instead.
- **Keyword Deduplication Enhancement (August 20, 2025)**: Implemented comprehensive keyword deduplication system using Set data structure to eliminate overlapping keywords across profileKeywords, equipmentKeywords, and workKeywords. Restructured query generation to use hierarchical priority (profile keywords > equipment tags > work keywords) and removed redundant equipment/work names that were already included in specificQuery. Enhanced category-specific search queries with deduplicated baseKeywords for more efficient and precise vector database searches. Added detailed logging system to track keyword integration and deduplication effectiveness.
- **Complex Logic Simplification (August 20, 2025)**: Successfully removed 131+ lines of complex keyword expansion logic from chroma-rag-service.ts functions (searchRelevantAccidents, searchSafetyRegulations, searchEducationMaterials, performKeywordSearch). Eliminated hardcoded equipment-specific keyword expansions that duplicated ai-service.ts profile-based keyword generation. Unified architecture now relies on vector DB search with simple keyword matching, removing scoring algorithms and complex filtering logic for improved maintainability.
- **Function Consistency Implementation (August 20, 2025)**: Standardized chroma-rag-service.ts search functions with unified patterns - consistent log messages, simplified return logic, and removed unnecessary initialization calls. Fixed ChromaClient import errors and type references, achieving clean TypeScript compilation. All three search functions (accidents, regulations, education) now follow identical patterns while maintaining legacy interface compatibility.
